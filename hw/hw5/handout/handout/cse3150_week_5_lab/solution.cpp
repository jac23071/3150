...
std::vector<int> compress_row(const std::vector<int>& row) {
	...
	//std::vector<int> compressed(4,0);
	//for (const auto& num: row){
	//	size_t comp_idx = 0;
	//	if (num != 0){
	//		compressed[comp_idx] = num;
	//		comp_idx++;
	//	}
	//}
	...
	for(int idx = 0; idx < 4; idx++){
		if (new_row[idx] == 0){
			for (int idx2 = idx; idx2 < 4; idx2++){
				if (new_row[idx2] != 0){
					new_row[idx] = row[idx2];
					new_row[idx2] = 0;
					break;
				}
			}
...
bool move_left(std::vector<std::vector<int>>& board) {
    ...
    for(auto& row: board){
    	std::vector<int> OG = row;
	row = compress_row(row);
	row = merge_row(row);
	row = compress_row(row);
	if (row != OG){moved = true;}
    }
    return moved;
}
...
bool move_up(std::vector<std::vector<int>>& board) {
    ...
    std::vector<std::vector<int>> rotated(4, std::vector(4,0));
    ...
    for(auto& col: rotated){
    	std::vector<int> OG = col;
	col = compress_row(col);
	col = merge_row(col);
	col = compress_row(col);
	if (col != OG){moved = true;}
    }

    for(size_t i = 0; i < 4; i++){
    	for(size_t j = 0; j < 4; j++){
		board[i][j] = rotated[j][i];
	}
    }
    return moved;
}
